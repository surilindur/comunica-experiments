...
class JbrQuery:
    ...
    def __init__(self, combination: str, row: Dict[str, str]) -> None:
        ...
        self.replication = int(row.get("replication", "1"))
        if row.get("timestampsAll"):
            # parse the new query runner format file
            try:
                self.timestamps = array([[t / 1000 for t in t_r] for t_r in loads(row["timestampsAll"])])
            except ValueError as ex:
                assert "inhomogeneous shape after 1" in str(ex)
        elif row.get("timestamps"):
            # parse old query runner timestamps format
            csv_timestamps = list(float(t) / 1000 for t in row["timestamps"].split(TIMESTAMP_DELIMITER))
            csv_timestamps.sort()
            self.timestamps = array([csv_timestamps])
        self.failed = len(self.timestamps.shape) < 2 or self.timestamps.shape[1] < 1
        self.duration_avg = float(row["time"]) / 1000
        self.duration_min = float(row["timeMin"]) / 1000 if "timeMin" in row else self.duration_avg
        self.duration_max = float(row["timeMax"]) / 1000 if "timeMax" in row else self.duration_avg
        self.results_avg = float(row["results"])
        self.results_min = float(row["resultsMin"]) if "resultsMin" in row else self.results_avg
        self.results_max = float(row["resultsMax"]) if "resultsMax" in row else self.results_avg
        self.http_requests_avg = float(row["httpRequests"] or "0")
        self.http_requests_min = float(row["httpRequestsMin"] or "0") if "httpRequestsMin" in row else self.http_requests_avg
        self.http_requests_max = float(row["httpRequestsMax"] or "0") if "httpRequestsMax" in row else self.http_requests_avg
        if not self.failed and row["error"] == "true":
            warning(f"Accepting failed query {self.name} with error {row.get("errorDescription")}")
