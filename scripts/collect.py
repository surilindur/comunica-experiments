...
def collect_results(path: Path) -> None:
    ...
    for combination in path.joinpath("combinations").iterdir():
        ...
        results_path = results_path.joinpath(combination.name)
        copytree(
        ...
        copy(
            ...
            results_path.joinpath(combination_manifest_path.name),
        )
        results_path.joinpath(".keep").unlink(missing_ok=True)

        # Compress the logs directory
        logs_directory_path = results_path.joinpath("logs")
        logs_archive_path = results_path.joinpath("logs.tar.gz")
        ...
        # Compress the stats
        stats_archive_path = results_path.joinpath("stats.tar.gz")
